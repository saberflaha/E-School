...
def selectRandomButton(testObjectPath, label) {
 ...
 boolean button_uint_questiona=  WebUI.verifyElementVisible(findTestObject('Object Repository/book index/Page_-   -  -   - joacademy.com/verify button_ uint questiona'))
 if(button_uint_questiona) {println('Pass')}else {println('Fail')}
   WebUI.closeBrowser()
